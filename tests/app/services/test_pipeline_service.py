...
# --- Tests for TEXT_CLASSIFIER --- 

def test_trigger_pipeline_flow_text_classifier_success(
    mock_db_session: MagicMock,
    mock_uploaded_file_log: UploadedFileLog
):
    """Test successful sync execution of a TEXT_CLASSIFIER pipeline."""
    mock_extracted_text_pages = [("This is technical text about Python and APIs.", 1)]
    mock_classifier_flow_result = {"status": "success", "category": "Technical", "message": "Text classified successfully."}
    
    mock_db_session._object_store[mock_uploaded_file_log.id] = mock_uploaded_file_log
    
    with patch("app.services.pipeline_service.extract_text_from_pdf", return_value=mock_extracted_text_pages) as mock_extract:
        with patch("app.services.pipeline_service.text_classification_flow", return_value=mock_classifier_flow_result) as mock_classify_flow:
            response = trigger_pipeline_flow(
                db=mock_db_session,
                uploaded_file_log_id=mock_uploaded_file_log.id,
                pipeline_type=PipelineType.TEXT_CLASSIFIER
            )
    ...
    assert response.run_uuid is not None
    assert response.status == PipelineRunStatus.COMPLETED
    assert "TEXT_CLASSIFIER flow executed synchronously" in response.message

    final_run_state = mock_db_session._object_store.get(response.run_uuid)
    assert final_run_state is not None
    assert final_run_state.status == PipelineRunStatus.COMPLETED
    assert final_run_state.pipeline_type == PipelineType.TEXT_CLASSIFIER
    assert final_run_state.result == mock_classifier_flow_result
    assert final_run_state.error_message is None

    mock_extract.assert_called_once_with(mock_uploaded_file_log.storage_location)
    expected_text_content = "This is technical text about Python and APIs."
    mock_classify_flow.assert_called_once_with(text_content=expected_text_content)

@pytest.mark.parametrize(
    "extracted_pages_mock_return, expected_error_message_part",
    [
        ([], "No text could be extracted"), # No text extracted
        ([("   ", 1)], "Extracted text is empty")    # Empty text extracted
    ]
)
def test_trigger_pipeline_flow_text_classifier_text_extraction_issues(
    mock_db_session: MagicMock,
    mock_uploaded_file_log: UploadedFileLog,
    extracted_pages_mock_return: list,
    expected_error_message_part: str
):
    """Test TEXT_CLASSIFIER pipeline when PDF text extraction yields no usable text."""
    mock_db_session._object_store[mock_uploaded_file_log.id] = mock_uploaded_file_log
    
    with patch("app.services.pipeline_service.extract_text_from_pdf", return_value=extracted_pages_mock_return) as mock_extract:
        with patch("app.services.pipeline_service.text_classification_flow") as mock_classify_flow: # Should not be called
            response = trigger_pipeline_flow(
                db=mock_db_session,
                uploaded_file_log_id=mock_uploaded_file_log.id,
                pipeline_type=PipelineType.TEXT_CLASSIFIER
    ...
    assert response.status == PipelineRunStatus.FAILED
    final_run_state = mock_db_session._object_store.get(response.run_uuid)
    assert final_run_state is not None
    assert final_run_state.status == PipelineRunStatus.FAILED
    assert expected_error_message_part in final_run_state.error_message
    mock_extract.assert_called_once_with(mock_uploaded_file_log.storage_location)
    mock_classify_flow.assert_not_called()

def test_trigger_pipeline_flow_text_classifier_failure_modes(
    mock_db_session: MagicMock,
    mock_uploaded_file_log: UploadedFileLog
):
    """Test TEXT_CLASSIFIER pipeline failures: flow raises exception or returns error status."""
    mock_db_session._object_store[mock_uploaded_file_log.id] = mock_uploaded_file_log
    mock_valid_extracted_text = [("Some text to classify.", 1)]

    # Case 1: Classifier flow function raises an exception
    classifier_flow_exception = Exception("Classifier flow processing error")
    with patch("app.services.pipeline_service.extract_text_from_pdf", return_value=mock_valid_extracted_text):
        with patch("app.services.pipeline_service.text_classification_flow", side_effect=classifier_flow_exception) as mock_classify_flow_exc:
            with pytest.raises(HTTPException) as exc_info_case1:
                trigger_pipeline_flow(
                    db=mock_db_session,
                    uploaded_file_log_id=mock_uploaded_file_log.id,
    ...
    assert exc_info_case1.value.status_code == 500
    assert str(classifier_flow_exception) in exc_info_case1.value.detail
    mock_classify_flow_exc.assert_called_once()
    failed_run_uuid_case1 = next((uid for uid, obj in mock_db_session._object_store.items() if isinstance(obj, PipelineRun) and obj.pipeline_type == PipelineType.TEXT_CLASSIFIER and str(classifier_flow_exception) in obj.error_message), None)
    assert failed_run_uuid_case1 is not None
    failed_run_state_case1 = mock_db_session._object_store[failed_run_uuid_case1]
    assert failed_run_state_case1.status == PipelineRunStatus.FAILED
    ...
    # Case 2: Classifier flow function returns error status
    mock_classifier_flow_error_result = {"status": "error", "message": "Specific classifier error"}
    with patch("app.services.pipeline_service.extract_text_from_pdf", return_value=mock_valid_extracted_text):
        with patch("app.services.pipeline_service.text_classification_flow", return_value=mock_classifier_flow_error_result) as mock_classify_flow_err:
            response_err = trigger_pipeline_flow(
                db=mock_db_session,
                uploaded_file_log_id=mock_uploaded_file_log.id,
                pipeline_type=PipelineType.TEXT_CLASSIFIER
    ...
    assert final_run_state_err.status == PipelineRunStatus.FAILED
    assert final_run_state_err.result is None
    assert mock_classifier_flow_error_result["message"] in final_run_state_err.error_message
    mock_classify_flow_err.assert_called_once()
